...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            node.queue_key = NodeDistancePair(node,float("inf"))
            node.parent = None
            pq.insert(node.queue_key) #inserting the nodes into priority queue
        ...
        while len(pq) > 0:
            current_node = pq.extract_min() #deletes the current node from priority queue
            node,dist = current_node.node,current_node.distance #using the NodeDistancePair attribute to create 2 variable
            num_visited = num_visited + 1

            if node == destination:
                break

            for neighbour in current_node.node.adj:                  #relax process
                new_distance = weight(node,neighbour) + dist  #calculate the new distance
                
                if neighbour.queue_key.distance > new_distance:    #checking to see if the new distance calculated is lower
                    neighbour.queue_key = NodeDistancePair(neighbour, new_distance)  #relaxing distance
                    neighbour.parent = node                                #setting a parent relationship for the adjacent node
                    pq.decrease_key(neighbour.queue_key) #decrease the key after node.distance changes


        path = []
        destination_placeholder = destination
        while destination_placeholder is not None:          #backtracking from destination node to source node. Ends when source node is reached
            path.append(destination_placeholder)
            destination_placeholder = destination_placeholder.parent #backtracking
